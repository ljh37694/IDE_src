...
// adjust servo position according to the USS read value
  ...
  myservo.writeMicroseconds(_DUTY_MIN + n * (int(dist_ema) - 180));
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if (reading < _DIST_MIN || reading > _DIST_MAX) reading = 0;
    
  if(reading < 180.0 || reading > 360.0) reading = dist_prev;
